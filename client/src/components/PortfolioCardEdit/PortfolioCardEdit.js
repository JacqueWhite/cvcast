    ...
    <div className="card portfolio-card">
      ...
      <div className="card-action">
        ...
        <span><EditButton onClick = {() => {props.edit(props)}}/>
          <a className="remove btn-floating waves-effect waves-light red" onClick={props.onClick} onClick = {() => props.remove(props.id)}><i className="material-icons">delete_forever</i></a></span>
      </div>
      <div className="card-reveal">
        <span className="card-title grey-text text-darken-4">
          <a href={props.link} target="_blank">{props.project}</a><i className="material-icons right">close</i>
        </span>
